...
"""
A function BoDau is provided by Luu Tuan Anh
"""

def is_viet_alpha(unicode_char):
    """
    If an input string includes vietnamese character, then return True.
    """
    if u''.join([BoDau(c) for c in unicode_char]).isalpha():
        return True
    return False

def delete_tonemark(string):
    """
    Delete diacritics marks(Tone marks) from an input string.
    """
